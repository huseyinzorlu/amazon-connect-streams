...
(function () {
  ...
  var requestIceAccess = function (transport) {
    return new Promise(function (resolve, reject) {
      connect.core
        .getClient()
        .call(connect.ClientMethods.CREATE_TRANSPORT, transport, {
          ...
          failure: function (reason) {
            if (
              ...
              reason.message.includes(
                "SoftphoneConnectionLimitBreachedException"
              )
            ) {
              publishError(
  ...
  var SoftphoneManager = function (softphoneParams = {}) {
    ...
    logger
      .info("[Softphone Manager] softphone manager initialization has begun")
      .sendInternalLogToServer();
    logger
      .info(
        `[SoftphoneManager] Client Provided Strategy: ${softphoneParams.VDIPlatform}`
      )
      .sendInternalLogToServer();
    ...
    if (softphoneParams.VDIPlatform) {
      try {
        if (softphoneParams.VDIPlatform === VDIPlatformType.CITRIX) {
          ...
          logger
            .info(
              `[SoftphoneManager] Strategy constructor retrieved: ${rtcJsStrategy}`
            )
            .sendInternalLogToServer();
        } else {
        ...
        if (error.message === "VDI Strategy not supported") {
          publishError(
            SoftphoneErrorTypes.VDI_STRATEGY_NOT_SUPPORTED,
            error.message,
            ""
          );
          throw error;
          ...
          publishError(
            SoftphoneErrorTypes.VDI_REDIR_NOT_SUPPORTED,
            error.message,
            ""
          );
          throw error;
    ...
    if (connect.RtcPeerConnectionFactory) {
      if (rtcJsStrategy) {
        rtcPeerConnectionFactory = new connect.RtcPeerConnectionFactory(
          logger,
    ...
    const onSetMicrophoneDeviceSub = handleMicrophoneDeviceChange(
      !softphoneParams.disableEchoCancellation
    );
    ...
    var isContactTerminated = function (contact) {
      return (
        contact.getStatus().type === connect.ContactStatusType.ENDED ||
      ...
      if (rtcSessions.hasOwnProperty(agentConnectionId)) {
        ...
        new Promise(function (resolve, reject) {
          ...
          lily
            .getLog()
            .warn(
              "Clean up the session locally " + agentConnectionId,
              err.message
            )
            .sendInternalLogToServer();
        });
      ...
      if (Object.keys(rtcSessions).length > 0) {
        ...
        for (var connectionId in rtcSessions) {
          if (rtcSessions.hasOwnProperty(connectionId)) {
            ...
            publishMultipleSessionsEvent(
              HANG_UP_MULTIPLE_SESSIONS_EVENT,
              rtcSessions[connectionId].callId,
              connectionId
            );
    ...
    this.startSession = function (_contact, _agentConnectionId) {
      ...
      var agentConnectionId = isSessionPending
        ? pendingAgentConnectionId
        : _agentConnectionId;
      ...
      logger
        .info(
          "Softphone call detected:",
          "contactId " + contact.getContactId(),
          "agent connectionId " + agentConnectionId
        )
        .sendInternalLogToServer();
      ...
      session.remoteAudioElement =
        document.getElementById("remote-audio") ||
        window.parent.parent.document.getElementById("remote-audio");
    ...
    var onRefreshContact = function (contact, agentConnectionId) {
      ...
      if (
        ...
        !callsDetected[agentConnectionId] &&
    ...
    new connect.Agent().getContacts().forEach(function (contact) {
      ...
      logger
        .info(
          "Contact exist in the snapshot. Reinitiate the Contact and RTC session creation for contactId" +
            contact.getContactId(),
          "agent connectionId " + agentConnectionId
        )
        .sendInternalLogToServer();
    ...
    this.terminate = () => {
      self.onInitContactSub &&
        self.onInitContactSub.unsubscribe &&
        self.onInitContactSub.unsubscribe();
      onMuteSub && onMuteSub.unsubscribe && onMuteSub.unsubscribe();
      onSetSpeakerDeviceSub &&
        onSetSpeakerDeviceSub.unsubscribe &&
        onSetSpeakerDeviceSub.unsubscribe();
      onSetMicrophoneDeviceSub &&
        onSetMicrophoneDeviceSub.unsubscribe &&
        onSetMicrophoneDeviceSub.unsubscribe();
  ...
  var fireContactAcceptedEvent = function (contact) {
    ...
    if (!agentConnection) {
      logger
        .info(
          "Not able to retrieve the auto-accept setting from null AgentConnection, ignoring event publish.."
        )
        .sendInternalLogToServer();
      return;
    }
    ...
    if (!softphoneMediaInfo) {
      logger
        .info(
          "Not able to retrieve the auto-accept setting from null SoftphoneMediaInfo, ignoring event publish.."
        )
        .sendInternalLogToServer();
      return;
    }
    if (softphoneMediaInfo.autoAccept === true) {
      logger
        .info(
          "Auto-accept is enabled, sending out Accepted event to stop ringtone.."
        )
        .sendInternalLogToServer();
      ...
      conduit.sendUpstream(connect.EventType.BROADCAST, {
        event: connect.core.getContactEventName(
          connect.ContactEvents.ACCEPTED,
          contact.contactId
        ),
        data: new connect.Contact(contact.contactId),
      });
      ...
      logger
        .info(
          "Auto-accept is disabled, ringtone will be stopped by user action."
        )
        .sendInternalLogToServer();
    }
  };
  ...
  var handleMicrophoneDeviceChange = function (enableEchoCancellation) {
    ...
    return bus.subscribe(
      ...
      (data) => setMicrophoneDevice({ ...data, enableEchoCancellation })
    );
  };
        ...
        if (localMedia) {
          ...
          if (status !== undefined) {
            ...
            if (status) {
              ...
              logger
                .info(
                  "Agent has unmuted the contact, connectionId - " +
                    connectionId
                )
                .sendInternalLogToServer();
            }
  ...
  var setSpeakerDevice = function (data = {}) {
    ...
    connect
      .getLog()
      .info(
        `[Audio Device Settings] Attempting to set speaker device ${deviceId}`
      )
      .sendInternalLogToServer();
    ...
    if (!deviceId) {
      connect
        .getLog()
        .warn(
          "[Audio Device Settings] Setting speaker device cancelled due to missing deviceId"
        )
        .sendInternalLogToServer();
      return;
    }

    var remoteAudioElement =
      document.getElementById("remote-audio") ||
      window.parent.parent.document.getElementById("remote-audio");
    if (
      remoteAudioElement &&
      ...
      remoteAudioElement
        ...
        .then(() => {
          connect
            .getLog()
            .info(
              `[Audio Device Settings] Speaker device ${deviceId} successfully set to speaker audio element`
            )
            .sendInternalLogToServer();
          connect.core.getUpstream().sendUpstream(connect.EventType.BROADCAST, {
        ...
        .catch((e) => {
          connect
            .getLog()
            .error(
              "[Audio Device Settings] Failed to set speaker device " + deviceId
            )
            .withException(e)
            .sendInternalLogToServer();
        });
      ...
      connect
        .getLog()
        .warn(
          "[Audio Device Settings] Setting speaker device cancelled due to missing remoteAudioElement"
        )
        .sendInternalLogToServer();
    }
  };
  ...
  var setMicrophoneDevice = function (data = {}) {
    ...
    connect
      .getLog()
      .info(
        `[Audio Device Settings] Attempting to set microphone device ${deviceId}`
      )
      .sendInternalLogToServer();
    ...
    if (connect.keys(localMediaStream).length === 0) {
      connect
        .getLog()
        .warn(
          "[Audio Device Settings] Setting microphone device cancelled due to missing localMediaStream"
        )
        .sendInternalLogToServer();
      return;
    }
    if (!deviceId) {
      connect
        .getLog()
        .warn(
          "[Audio Device Settings] Setting microphone device cancelled due to missing deviceId"
        )
        .sendInternalLogToServer();
      return;
    }
        ...
        try {
          ...
          for (var connectionId in localMediaStream) {
            if (localMediaStream.hasOwnProperty(connectionId)) {
              ...
              session._pc
                .getSenders()[0]
                .replaceTrack(newMicrophoneTrack)
                .then(function () {
                  //Replace the audio track in the local media stream (for mute / unmute)
                  softphoneManager.replaceLocalMediaTrack(
                    connectionId,
                    newMicrophoneTrack
                  );
                  connect
                    .getLog()
                    .info(
                      `[Audio Device Settings] Microphone device ${deviceId} successfully set to local media stream in RTCRtpSender`
                    )
                    .sendInternalLogToServer();
                });
            }
          ...
          connect
            .getLog()
            .error(
              "[Audio Device Settings] Failed to set microphone device " +
                deviceId
            )
            .withException(e)
            .sendInternalLogToServer();
  ...
  var publishSoftphoneFailureLogs = function (rtcSession, reason) {
    if (reason === connect.RTCErrors.ICE_COLLECTION_TIMEOUT) {
      ...
      publishError(
        SoftphoneErrorTypes.ICE_COLLECTION_TIMEOUT,
      ...
      publishError(
        ...
        "Handshaking with Signalling Server " +
          rtcSession._signalingUri +
          " failed. ",
        rtcSession._signalingUri
      );
    } else if (
      reason === connect.RTCErrors.GUM_TIMEOUT_FAILURE ||
      reason === connect.RTCErrors.GUM_OTHER_FAILURE
    ) {
      publishError(
        SoftphoneErrorTypes.MICROPHONE_NOT_SHARED,
        "Your microphone is not enabled in your browser. ",
      ...
      logger
        .error("Softphone call failed due to CallNotFoundException.")
        .sendInternalLogToServer();
    } else {
      publishError(
  ...
  var publishError = function (errorType, message, endPointUrl) {
    logger
      .error("Softphone error occurred : ", errorType, message || "")
      .sendInternalLogToServer();
  ...
  var publishMultipleSessionsEvent = function (
    eventName,
    contactId,
    agentConnectionId
  ) {
    ...
    logger
      .info(
        "Publish multiple session error metrics",
        eventName,
        "contactId " + contactId,
        "agent connectionId " + agentConnectionId
      )
  ...
  SoftphoneManager.isBrowserSoftPhoneSupported = function () {
    ...
    // In Chrome, the true version is after "Chrome"
      ...
      connect.isChromeBrowser() &&
      connect.getChromeBrowserVersion() > 22
    ) {
      return true;
    }
    // In Firefox, the true version is after "Firefox"
    else if (
      connect.isFirefoxBrowser() &&
      connect.getFirefoxBrowserVersion() > 21
    ) {
      return true;
    } else {
      return false;
  ...
  var sendSoftphoneMetrics = function (contact) {
    ...
    if (streamStats.length > 0) {
      contact.sendSoftphoneMetrics(streamStats, {
        success: function () {
          logger
            .info("sendSoftphoneMetrics success" + JSON.stringify(streamStats))
            .sendInternalLogToServer();
        },
        failure: function (data) {
          logger
            .error("sendSoftphoneMetrics failed.")
            .withObject(data)
            .sendInternalLogToServer();
        },
      });
  ...
  var sendSoftphoneReport = function (
    contact,
    report,
    userAudioStats,
    remoteAudioStats
    ...
    contact.sendSoftphoneReport(callReport, {
      success: function () {
        logger
          .info("sendSoftphoneReport success" + JSON.stringify(callReport))
          .sendInternalLogToServer();
      },
      failure: function (data) {
        logger
          .error("sendSoftphoneReport failed.")
          .withObject(data)
          .sendInternalLogToServer();
      },
    });
    ...
    var streamPerSecondStats = {
      AUDIO_INPUT: {
        packetsCount: inputRTPStreamStatsBuffer.map(
          (stats) => stats.packetsCount
        ),
        packetsLost: inputRTPStreamStatsBuffer.map(
          (stats) => stats.packetsLost
        ),
        audioLevel: inputRTPStreamStatsBuffer.map((stats) => stats.audioLevel),
        jitterBufferMillis: inputRTPStreamStatsBuffer.map(
          (stats) => stats.jitterBufferMillis
        ),
      },
      AUDIO_OUTPUT: {
        packetsCount: outputRTPStreamStatsBuffer.map(
          (stats) => stats.packetsCount
        ),
        packetsLost: outputRTPStreamStatsBuffer.map(
          (stats) => stats.packetsLost
        ),
        audioLevel: outputRTPStreamStatsBuffer.map((stats) => stats.audioLevel),
        jitterBufferMillis: outputRTPStreamStatsBuffer.map(
          (stats) => stats.jitterBufferMillis
        ),
        roundTripTimeMillis: outputRTPStreamStatsBuffer.map(
          (stats) => stats.roundTripTimeMillis
        ),
      },
    };
  ...
  var startStatsCollectionJob = function (rtcSession) {
    rtpStatsJob = window.setInterval(function () {
      rtcSession.getUserAudioStats().then(
        ...
        function (error) {
          logger
            .debug("Failed to get user audio stats.", error)
            .sendInternalLogToServer();
        }
      );
      rtcSession.getRemoteAudioStats().then(
        function (stats) {
          var previousRemoteStats = aggregatedRemoteAudioStats;
          aggregatedRemoteAudioStats = stats;
          var currRTPStreamStat = getTimeSeriesStats(
            aggregatedRemoteAudioStats,
            previousRemoteStats,
            AUDIO_OUTPUT
          );
          timeSeriesStreamStatsBuffer.push(currRTPStreamStat);
        ...
        function (error) {
          logger
            .debug("Failed to get remote audio stats.", error)
            .sendInternalLogToServer();
        }
      );
    }, 1000);
  ...
  var telemetryCallReportRTPStreamStatsBuffer = function (rtpStreamStats) {
    if (rtpStreamStats.softphoneStreamType === AUDIO_INPUT) {
      while (
        inputRTPStreamStatsBuffer.length >= MAX_RTP_STREAM_STATS_BUFFER_SIZE
      ) {
      ...
      while (
        outputRTPStreamStatsBuffer.length >= MAX_RTP_STREAM_STATS_BUFFER_SIZE
      ) {
  ...
  var stopJobsAndReport = function (contact, sessionReport) {
    ...
    sendSoftphoneReport(
      contact,
      sessionReport,
      addStreamTypeToStats(aggregatedUserAudioStats, AUDIO_INPUT),
      addStreamTypeToStats(aggregatedRemoteAudioStats, AUDIO_OUTPUT)
    );
    sendSoftphoneMetrics(contact);
  };
  ...
  var SoftphoneLogger = function (logger) {
    ...
    this._tee = function (level, method) {
      return function () {
        ...
        return method.apply(
          self._originalLogger,
          [connect.LogComponent.SOFTPHONE, format].concat(args)
        );
      };
    };
  };
